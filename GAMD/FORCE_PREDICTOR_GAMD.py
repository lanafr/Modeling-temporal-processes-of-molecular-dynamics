...
from nn_module import GAMD, SimpleMDNetNew
from train_utils_seq import LJDataNew
...
class ParticleNetLightning(pl.LightningModule):
    ...
    def training_step(self, batch, batch_nb):
        ...
        pred = self.pnet_model(pos_lst[0])
        #                       edge_idx_lst,
        #                       )
        ...
        self.log('var', np.sqrt(self.training_var).item(), on_step=True, prog_bar=True, logger=True)
    ...
    def train_dataloader(self):
        dataset = LJDataNew(dataset_path=os.path.join(self.data_dir, 'lj_data_20ts'),
                               sample_num=1000,
                               case_prefix='data_',
                               seed_num=10,
                               mode='train')
    ...
    def val_dataloader(self):
        ...
        return DataLoader(dataset, num_workers=2, batch_size=1, shuffle=False,
                          collate_fn=
                          lambda batches: {
                              'pos': [batch['pos'] for batch in batches],
                              'forces': [batch['forces'] for batch in batches],
    ...
    def validation_step(self, batch, batch_nb):
        with torch.no_grad():
            ...
            outlier_ratio = torch.tensor(ratio[ratio > 10.].shape[0] / ratio.shape[0])
            mse = nn.MSELoss()(pred, gt)
            mae = nn.L1Loss()(pred, gt)

            self.log('val outlier', outlier_ratio.item(), prog_bar=True, on_epoch=True, logger=True, batch_size=1)
            self.log('val mse', mse, prog_bar=True, logger=True, on_epoch=True, batch_size=1)
            self.log('val mae', mae, prog_bar=True, logger=True, on_epoch=True, batch_size=1)
...
class ModelCheckpointAtEpochEnd(pl.Callback):
    ...
    def on_train_epoch_end(self, trainer: pl.Trainer, pl_module: ParticleNetLightning):
...
def train_model(args):
    ...
    trainer.fit(model, ckpt_path='./model_ckpt/GAMDwithJAX_new/checkpoint_20.ckpt')


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument('--min_epoch', default=35, type=int)
    parser.add_argument('--max_epoch', default=35, type=int)
    parser.add_argument('--lr', default=1e-4, type=float)
    parser.add_argument('--cp_dir', default='./model_ckpt/GAMDwithJAX_new')
