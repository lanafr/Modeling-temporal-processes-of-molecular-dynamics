...
class MDSimNet(pl.LightningModule):
    ...
    def apply_minimum_image_convention(self, r, box_length):
        # r is the displacement vector (predicted - actual)
        # box_length is the tensor of box dimensions
        return r - box_length * torch.round(r / box_length)
    
    def cosine_similarity_loss(self, actual_t0, actual_t1, predicted_t1, box_length):
        # Calculate displacement using MIC
        displacement_actual = self.apply_minimum_image_convention(actual_t1 - actual_t0, box_length)
        displacement_predicted = self.apply_minimum_image_convention(predicted_t1 - actual_t0, box_length)

        # Normalize the vectors (avoiding zero division)
        norm_actual = torch.linalg.norm(displacement_actual, dim=1, keepdim=True)
        norm_predicted = torch.linalg.norm(displacement_predicted, dim=1, keepdim=True)
        
        displacement_actual_normalized = displacement_actual / norm_actual.clamp(min=1e-8)
        displacement_predicted_normalized = displacement_predicted / norm_predicted.clamp(min=1e-8)

        # Cosine similarity per item
        cosine_sim = (displacement_actual_normalized * displacement_predicted_normalized).sum(dim=1)

        # Cosine loss (you can also use 1 - cosine_sim.mean() depending on your requirement)
    ...
    def training_step(self, batch, batch_nb):
        ...
        temp_predicted = self.get_temp_from_vel(notnor_vel)
        temp_actual = self.get_temp_from_vel(notnor_orig_vel)
        
        cosine_loss = self.cosine_similarity_loss(pos_orig, pos_lst, pos_res, box_length)
       
        loss = vel_loss + cord_loss + cosine_loss
        ...
        self.log('cosine_loss', cosine_loss, on_step=True, prog_bar=True, logger=True)
        self.log('temp_pred', temp_predicted, on_step=True, prog_bar=True, logger=True)
        self.log('actual_temp', temp_actual, on_step=True, prog_bar=True, logger=True)
        self.log(f'actual loss:{self.loss_fn}', loss, on_step=True, prog_bar=True, logger=True)
        return loss

    def configure_optimizers(self):
            optim = torch.optim.Adam(self.parameters(), lr=self.learning_rate)
            sched = StepLR(optim, step_size=5, gamma=0.001**(5/self.epoch_num)) ## changed 5 to 10
            return [optim], [sched]
    ...
    def validation_step(self, batch, batch_nb):
        ...
        with torch.no_grad():
            ...
            self.log('cosine_vel_loss', cosine_loss, on_step=True, prog_bar=True, logger=True, batch_size=1)
...
def train_model(args):
    ...
    trainer = Trainer(devices=num_gpu, #changed from gpus
...
def main():
    ...
    parser.add_argument('--cp_dir', default='./model_ckpt/one')
